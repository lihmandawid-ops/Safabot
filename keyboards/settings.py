...
def settings_home_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(t("settings.menu.language", _LANG), callback_data="set:lang:list")],
            [InlineKeyboardButton(t("settings.menu.interface_language", _LANG), callback_data="set:iface:list")],
            [InlineKeyboardButton(t("settings.menu.daily_words", _LANG), callback_data="set:words:list")],
...
def back_to_settings_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton(t("settings.menu.back", _LANG), callback_data="set:home")]]
    )
...
def language_switch_keyboard(user_languages, *, can_add_more: bool) -> InlineKeyboardMarkup:
    ...
    for ul in user_languages:
        ...
        label = f"{lang.flag} {lang.name_ru}" if lang else ul.language_code
        if ul.is_current:
            label = f"✅ {label}"
...
def interface_language_pick_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        InlineKeyboardButton(f"{lang.flag} {lang.name_ru}", callback_data=f"set:iface:pick:{lang.code}")
...
def notification_slot_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(t("notification_slot.morning", _LANG), callback_data="set:notif:slot:morning")],
            [InlineKeyboardButton(t("notification_slot.afternoon", _LANG), callback_data="set:notif:slot:afternoon")],
            [InlineKeyboardButton(t("notification_slot.evening", _LANG), callback_data="set:notif:slot:evening")],
            [InlineKeyboardButton(t("settings.menu.back", _LANG), callback_data="set:home")],
        ]
    )
...
def level_pick_keyboard() -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(t(f"level.{code}", _LANG), callback_data=f"set:level:pick:{code}")]
        for code in LEVEL_CODES
    ]
    rows.append([InlineKeyboardButton(t("settings.menu.back", _LANG), callback_data="set:home")])
    return InlineKeyboardMarkup(rows)
