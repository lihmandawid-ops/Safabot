...
│   ├── dictionary_service.py      # локальный поиск + DictionaryProvider fallback (реализовано)
│   ├── ai_word_schema.py           # строгая валидация JSON от AI (word/generation) (реализовано)
...
│   ├── ai_service.py               # AIService: analyze_word/generate_words/... (интерфейс, без реального провайдера)
...
└── tests/
    ...
    │   test_manual_add_flow.py    # реализовано
```
...
3. Если провайдер что-то нашёл — результат валидируется
   (`services/ai_word_schema.py`) и сохраняется как обычная запись `Word`
   (со своими `WordTranslation`/`WordExample`), так что следующий поиск
   того же слова снова находит его локально, без повторного обращения к
   провайдеру.
4. Если ни локально, ни у провайдера ничего нет — пользователь видит
   честное «не найдено», а не пустой список без объяснений.

Без настроенного AI-провайдера (`AI_PROVIDER=none`, значение по
умолчанию) шаг 2 всегда возвращает «нет результата» — это ожидаемо и не
ошибка: работает только локальный словарь.
...
2. **AI — только на недостачу.** Если локального пула не хватило,
   ...
   generate_words()` (см. `services/ai_service.py`) — не больше.
3. **Строгая валидация.** Ответ AI обязан соответствовать схеме
   `{"words": [{"word", "translation": [...], "part_of_speech",
   "phonetic", "examples": [{"text", "translation"}], "difficulty",
   "category"}]}` — проверяется `services/ai_word_schema.
   parse_generation_response()`. Слово без `word` или без переводов не
   попадает в базу вообще; остальные поля отбрасываются, если содержат
   недопустимые значения (например, `part_of_speech` не из списка).
4. **Никогда не ломает обучение.** Любая ошибка AI-провайдера (не
   настроен, сеть недоступна, невалидный JSON) перехватывается — в ответ
...
5. **Каждый вызов логируется** в `WordGenerationLog` (user_id,
   ...
   created_at) — для контроля использования AI и затрат, даже если
   провайдер ни разу не был вызван (`provider="local"`).
...
### Как подключить настоящего AI-провайдера

1. В `.env` укажите `AI_PROVIDER` (например `openai`) и `AI_API_KEY`.
2. Реализуйте `services/ai_service.AIService` для этого провайдера (сейчас
   есть только `NotConfiguredAIService`, которая всегда бросает
   `NotImplementedError`) и подключите его в `get_ai_service()`.
3. Никакой код в `handlers/`, `dictionary_service.py` или
   `word_generation_service.py` менять не нужно — оба уже работают через
   единый интерфейс `AIService`/`DictionaryProvider`.
