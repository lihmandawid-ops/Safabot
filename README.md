...
| `SUPPORT_CONTACT` | нет | Контакт оператора (`@username` или `t.me/...`), показывается в ⚙️ Настройки → 🆘 Поддержка и в `/help`. Пусто = оба экрана показывают «поддержка не настроена» вместо пустого/битого контакта. |
